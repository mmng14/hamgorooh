...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $page_content = $ROOT_DIR . "app/admin/views/ads.view.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["SEND_TO_SUBJECTS_CODE"]) {
    ...
    try {
        ...
        if ($tblId != "") {
            Connect();
            $ads = select('ads', '*', "id='$tblId'");
            $subject_sites = select('subject_sites', '*', "");
            Disconnect();
            foreach ($subject_sites as $subject_site) {
                $title = $ads[0]['title'];
                $start_date = $ads[0]['start_date_num'];
                $end_date = $ads[0]['end_date_num'];
                $link_address = $ads[0]['link_address'];
                $type = $ads[0]['type'];
                $price = $ads[0]['price_per_day'];
                $photo = $ads[0]['photo'];
                $desc = $ads[0]['description'];
                $ordering = $ads[0]['ordering'];
                $status = $ads[0]['status'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]) {
    ...
    try {
        ...
        $property = array(
            "id" => null, "title" => $title, "start_date" => $start_date, "end_date" => $end_date, "start_date_num" => $start_date_num, "end_date_num" => $end_date_num, "link_address" => $link_address,
            "type" => $type, "price_per_day" => $price, "photo" => $photo, "description" => $desc,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    try {
        ...
        $property = array(
            "id" => $ads_id, "title" => $title, "start_date" => $start_date, "end_date" => $end_date, "start_date_num" => $start_date_num, "end_date_num" => $end_date_num, "link_address" => $link_address,
            "type" => $type, "price_per_day" => $price, "photo" => $photo, "description" => $desc,
            "ordering" => $ordering, "status" => $status
        );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && 
    ...
    $html = view_to_string("_ads.php", "app/admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
