...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $page_content = $ROOT_DIR . "app/admin/views/gallery.view.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["DELETE_CODE"]) {
    ...
    try {
        if ($tblId != "") {
            ...
            $delete_row = $database->gallery[$tblId];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["ACTIVATE_CODE"]) {
    ...
    if ($tblId != "") {
        $update_row = $database->gallery[$tblId];
        $status = 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&isset($_POST["check"]) && $_POST["check"] == $_SESSION["EDIT_CODE"]) {
    ...
    $gallery_title = $gallery_brief_desc = $gallery_photo = $gallery_desc = $gallery_ordering = $gallery_status = "";
    $tblId = test_input($_POST["obj"]);

    if ($tblId != "") {
        $result = $database->gallery()
            ->select("*")
            ->where("id=?", $tblId)
            ->fetch();
        ...
        if ($result) {
            ...
            $gallery_photo = $result['photo'];
            $gallery_desc = $result['description'];
            $gallery_link = $result['link'];
            $gallery_ordering = $result['ordering'];
            $gallery_status = $result['status'];
        }
        echo json_encode(
            array(
                "title" => $gallery_title,
    ...
    try {
        ...
        if (isset($_FILES['upload']) && $_FILES['upload']['error'] != UPLOAD_ERR_NO_FILE && is_uploaded_file($_FILES['upload']['tmp_name'])) {
            ...
            if (isValidImageExtension($extension)) {
                ...
                $newGUID = GUIDv4();
                $photo = "uploads/admin/gallery/" . $newGUID . $extension;
        ...
        $gallery = $database->gallery()->insert($property);
        $file_id = $gallery['id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    try {
        ...
        $gallery_id = mysql_escape_mimic($_POST['hashId']);
        $title = mysql_escape_mimic($_POST['title']);
        $desc = nl2br(mysql_escape_mimic($_POST['description']));
        $link = mysql_escape_mimic($_POST['link']);
        $ordering = mysql_escape_mimic($_POST['ordering']);
        ...
        $edit_row = $database->gallery[$gallery_id];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&    isset($_POST["cmd"]) && $_POST["cmd"] == $_SESSION["LISTING_CODE"] && isset($_SERVER['HTTP_X_REQUESTED_WITH']) &&
    ...
    $html = view_to_string("_gallery.php", "app/admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
