...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($url_cat_id)) {
        ...
        $request_category_string = $url_cat_id;
    ...
    $page_content = $ROOT_DIR . "app/admin/views/post_add.view.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&   isset($_POST["check"]) && $_POST["check"] == $_SESSION["POST_IMAGE_LIST_CODE"]) {
    ...
    $html = view_to_string("_photoList.php", "app/admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&   isset($_POST["check"]) && $_POST["check"] == $_SESSION["POST_RESOURCE_LIST_CODE"]) {
    ...
    $html = view_to_string("_resourceList.php", "app/admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
...
if (
    $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]
) {
    ...
    $update_date = '';
    $parent = mysql_escape_mimic($_POST['parent_id']);;
    $has_child = 0;
    $guid = GUIDv4();
    $post_type = 0;
    ...
    $post_array = array(
        ...
        "comment_status" => $comment, "reg_date" => $reg_date,
    ...
    if ($post_id == null || $post_id == '') {
        ...
        $redirect =  "{$HOST_NAME}admin/posts/{$this_category_id}/{$page_number}";
        echo json_encode(
            array(
                "status" => '1',
                "message" => $msg_ok,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    $attachment = mysql_escape_mimic($_POST['has_attachment']);
    $source_name = mysql_escape_mimic($_POST['source_name']);
    $source_link = mysql_escape_mimic($_POST['source_link']);
    $comment = mysql_escape_mimic($_POST['comment_status']);
    $post_status = mysql_escape_mimic($_POST['post_status']);
    ...
    $post_array = array(
        ...
        "post_status" => $post_status, "parent_id" => $parent_id,
        "photo_address" => $photo_address, "thumb_address" => $thumb_address,
