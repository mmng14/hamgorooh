...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $page_content = $ROOT_DIR . "app/admin/views/reportage_request.view.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["DELETE_CODE"]) {
    ...
    try {
        if ($tblId != "") {
            ...
            if ($delete_row) {
                $file_address = $delete_row['file_address'];

                $path = $RELATIVE_UPLOAD_FOLDER_PREFIX . $file_address;
                if (is_file($path)) {
                    unlink($path);
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["ACTIVATE_CODE"]) {
    ...
    if ($tblId != "") {
        $update_row = $database->reportage_request[$tblId];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["ACTIVATE_PAYMENT_CODE"]) {
    ...
    if ($tblId != "") {
        ...
        if ($update_row && $update_row["post_id"] && $update_row["subject_id"]) {
            $reportage_post_id = $update_row["post_id"];
            $reportage_subject_id = $update_row["subject_id"];
            $reportage_price = $update_row["price"];

            date_default_timezone_set('Asia/Tehran');
            $today_date = convertPersianToEng(jdate('Y/m/d'));
            $database_subject = getSubjectDatabase($reportage_subject_id);

            $newReportageId = moveToReportageTable($database, $database_subject, $reportage_post_id, $reportage_price, $today_date, $HOST_NAME);

            if (isset($newReportageId) && $newReportageId != null && $newReportageId != 0) {
                $payment_status = 1;
                $property = array("id" => $tblId, "payment_status" => $payment_status);
                $affected = $update_row->update($property);
                $deletedReportageId = removeFromReportageTable($database, $tblId);
                echo json_encode(
                    array(
                        "payment_state" => '1',
                    )
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["EDIT_CODE"]) {
    ...
    if ($tblId != "") {
        ...
        if ($result) {
            ...
            $reportage_link = $result['reportage_link'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]) {
    ...
    try {
        ...
        $reportage_link = mysql_escape_mimic($_POST['reportage_link']);
        ...
        $property = array(
            ...
            "customer_email" => $customer_email, "register_date" => $register_date, "reportage_link" => $reportage_link, "post_id" => $post_id, "status" => $status
        );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    try {
        ...
        $property = array(
            ...
            "price" => $price, "file_address" => $file_address, "user_id" => $user_id, "customer_name" => $customer_name,
            "customer_email" => $customer_email, "reportage_link" => $reportage_link, "post_id" => $post_id
...
if (
    ...
    $html = view_to_string("_reportage_request.php", "app/admin/views/partial/", $viewModel, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}


function moveToReportageTable($database, $database_subject, $post_id, $price, $today_date, $host_name)
{
    ...
    if ($post_id != "") {
        ...
        if ($reportage_find) {
            ...
            $reportage_post = array(
                ...
                "photo_address" => $reportage_photo_address, "thumb_address" => $reportage_thumb_address, "link_address" => $reportage_link, "post_date" => $post_date, "reg_date" => $reg_date, "featured" => "1", "visit" => "0", "price" => $price, "keywords" => $keywords
            );
...
function removeFromReportageTable($database, $request_id)
{
    ...
    if ($tblId != "") {
        ...
        if ($result) {
            ...
            $reportage_id = $result['id'];

            $delete_row = $database->reportage[$reportage_id];
            $affected = $delete_row->delete();
