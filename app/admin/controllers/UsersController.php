...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $page_content = $ROOT_DIR . "app/admin/views/users.view.php";
...
//**********************************  Set User Type  *******************************
//*************************************************************************************
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["userId"]) && isset($_POST["role"]) && isset($_POST["check"]) && $_POST["check"]==$_SESSION["SET_USER_TYPE_CODE"])
{
    page_access_check_ajax(array(1), $HOST_NAME);
    csrf_validation_ajax($_POST["_csrf"]);
	...
	if ($userId != "")
	{
        ...
        $update_row = $database->users[$userId];
        $status = 0;
        $property = array("id" => $userId, "type" => $role);
        $affected = $update_row->update($property);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]) {;
    ...
    try {
        ...
        $type  = mysql_escape_mimic($_POST['type']);
        ...
        $property = array("id" => null,"name"=>$fname,"family"=>$lname,"email"=>$email,"username"=>$user,
            "password"=>"","phone"=>$tell,"gender"=>$gender,"birth_date"=>$birth_date,"address"=>$address,"photo"=>$photo,
            "reg_date"=>$current_time,"type"=>$type,"status"=>$status);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    try {
        ...
        $property = array("name"=>$fname,"family"=>$lname,"email"=>$email,"username"=>$user,
        "password"=>"","phone"=>$tell,"gender"=>$gender,"birth_date"=>$birth_date,"address"=>$address,"photo"=>$photo,
...
if (
    ...
    $html = view_to_string("_users.php", "app/admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
