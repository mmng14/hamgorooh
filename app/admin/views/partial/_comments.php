<div class="table-responsive mailbox-messages">
    <table class="table table-hover table-striped">
        <thead>
            <tr>
                <th>ردیف</th>
                <th class="hidden-480">پست</th>
                <th class="hidden-phone">نویسنده نظر</th>
                <th class="hidden-phone">نظر</th>
            ...
            <?php if (isset($model)) : ?>
                <?php foreach ($model as $result) : ?>
                    <tr>
                        ...
                        <?php
                        $link_post_id =   $result['post_id'];
                        $link_post_id_length =  strlen($result['post_id']);
                        $link_subject_id =   $result['post_subject_id'];
                        $link_subject_id_length =  strlen($result['post_subject_id']);
                        $link_category_id =   $result['post_category_id'];
                        $link_category_id_length =  strlen($result['post_category_id']);
                        $link_post_name = "-";
                        ...
                        <td class="mailbox-subject"><a href="<?= $link_post_url ?>" target="_blank"><i class="fa fa-3x fa-link"></i></a></td>
                        <td class="mailbox-subject"><b><?= $result['author_name']; ?></b></td>
                        <td class="mailbox-subject"><b><?= $result['content']; ?></b></td>
                        <td class="mailbox-subject"><b><?= $result['author_email']; ?></b></td>
                        <td class="mailbox-subject"><b><?= $result['reg_date']; ?></b></td>
