            ...
            <?php if (isset($model)) : ?>
                <?php foreach ($model as $result) :  ?>
                    ...
                    <tr>
                        ...
                        <td><?= ++$counter ?></td>
                        <td class="hidden-480"><a href="<?= $link_post_url ?>" target="_blank"><?= $result['title'] ?></a></td>
                        <td class="hidden-phone">
                            <img alt="" src="<?= $thumb_address ?>" style="height: 45px !important;" />
                        </td>
                        ...
                        <td>
                            <a class="btn btn-green btn-sm btn-link-custom" title="ارسال به سایت مرتبط" onclick="sendToSubjectSite('<?= $result['id'] ?>')"><i class="fa fa-paper-plane  list-ctrl-icon" aria-hidden="true"></i></a>
                            <a class="btn btn-primary btn-sm btn-link-custom" title="ارسال به رپرتاژ" onclick="reportage('<?= $result['id'] ?>')"><i class="fa fa-paper-plane  list-ctrl-icon" aria-hidden="true"></i></a>
                            <a class="btn btn-blue btn-sm btn-link-custom" title="ارسال به تلگرام" onclick="telegram('<?= $result['id'] ?>')"><i class="fa fa-paper-plane  list-ctrl-icon" aria-hidden="true"></i></a>

                            <?php //if ($result['has_attachment'] == 1) : 
                            ?>
                            <a class="btn btn-green btn-sm btn-link-custom" href="<?= $HOST_NAME?>admin/post-attachments/<?= $link_subject_id ?>/<?= $link_category_id ?>/<?= $result['id']; ?>">
                                <i class="fa fa-paperclip list-ctrl-icon"></i>
                            </a>
                            <?php //endif;  
                            ?>

                            <a class="btn btn-purple btn-sm btn-link-custom" data-toggle="modal" href="<?php echo $HOST_NAME; ?>/admin/comments/<?= $link_subject_id ?>/<?= $link_category_id ?>/<?= $result['id'] ?>/">
                                <i class="fa fa-comment  list-ctrl-icon"></i>
                            </a>

                            <a href="<?php echo $HOST_NAME; ?>admin/post_add/<?php echo $link_category_id; ?>/<?php echo $link_post_id; ?>" class="btn btn-primary btn-sm btn-link-custom">
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                ...
                <?php if ($current_page < $total_pages) : ?>
                    ...
                    <li class="page-item">
                        <a class="page-link" href="#" data-page="<?= $next_link; ?>" title="Next">
                            < </a>
                    </li>
                    <li class="page-item last">
                        <a class="page-link" href="#" data-page="<?= $total_pages; ?>" title="Last">
                            << </a>
                    </li>
                <?php endif; ?>
