<div class="table-responsive mailbox-messages">
    <table class="table table-hover table-striped">
        <thead>
            <tr>
                <th>ردیف</th>
                <th class="hidden-480">نام</th>
                <th class="hidden-480">کد موضوع</th>
                <th class="hidden-480">کد</th>
                <th class="hidden-phone">نام داده</th>
                <th class="hidden-phone">نوع گروه</th>
                <th class="hidden-phone">تصویر</th>
                <th class="hidden-phone">سفارشی</th>
                <th class="hidden-phone">وضعیت</th>
                <th class="hidden-phone">زیر گروه</th>
                <th></th>
            </tr>
        </thead>
        <tbody>
            ...
            <?php if (isset($model)) : ?>
                <?php foreach ($model as $result) : ?>
                    <tr>
                        <?php
                        $category_id = encode_url($result["id"]);
                        //Get Thumb File
                        $arrThumb = array();
                        $arrThumb = explode("/", $result['photo']);
                        $arrThumb[count($arrThumb) - 1] = "thumb_" . $arrThumb[count($arrThumb) - 1];
                        $thumb_address = "";
                        $thumb_address = implode("/", $arrThumb);
                        $category_type = "";
                        if ($result['category_type'] == '1') $category_type = "پست";
                        if ($result['category_type'] == '2') $category_type = "کتاب";
                        if ($result['category_type'] == '3') $category_type = "مقاله";
                        ?>
                        <td><?= ++$counter ?></td>
                        ...
                        <td class="mailbox-subject"><b><?= $result['data_name']; ?></b></td>
                        <td class="mailbox-subject"><b><?= $category_type ?></b></td>

                        <td class="mailbox-subject">
                            <img alt="" src="<?= $HOST_NAME . $result['photo'] ?>" width="90px" height="50px" />
                        </td>
                        <td id="custom_<?= $result['id'] ?>" class="hidden-phone">
                            <?php if ($result['is_custom'] == 1) : ?>
                                <span  onClick="uncustomize(<?= $result['id'] ?>);"><i class="fa  fa-toggle-on active-toggle"></i></span>
                            <?php else : ?>
                                <span  onClick="customize(<?= $result['id'] ?>);"><i class="fa  fa-toggle-off deactive-toggle"></i></span>
                            <?php endif  ?>
                        </td>
                        ...
                        <td class="hidden-phone">
                            <a href="<?= $HOST_NAME ?>/admin/sub_category/<?= $result['id'] ?>"> زیر گروه </a>
                        </td>
                        <td class="mailbox-date">

                            <button class="btn btn-green btn-sm" onclick="sendToSubjects('<?= $result['id'] ?>')" title=" ارسال به سایت ">
                                <i class="fa fa-paper-plane list-ctrl-icon"></i>
                            </button>
                            <button class="btn btn-danger btn-sm" onclick="deleteToSubjects('<?= $result['id'] ?>')" title=" حذف از سایت ">
                                <i class="fa fa-paper-plane list-ctrl-icon"></i>
                            </button>
