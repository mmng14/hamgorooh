...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $page_content = $ROOT_DIR . "app/group_admin/views/crawler_items.view.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["EDIT_CODE"]) {
    ...
    try {
        ...
        $source_id = $subject_id = $category_id = $sub_category_id = $item_link = $crawler = $source_name = $source_link = "";
        $tblId = test_input($_POST["obj"]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]) {
    ...
    try {
        ...
        $property = array(
            ...
            "site_name" => $site_name, "crawler" => $crawler, "source_name" => $source_name, "source_link" => $source_link,"item_link" => $item_link,
            "status" => $status
        );
...
if (
    ...
    $html = view_to_string("_crawler_items.php", "app/group_admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
