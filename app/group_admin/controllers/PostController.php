...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($url_subject_id) && is_numeric($url_subject_id)) {
        ...
        $this_subject_id = $selected_subject_id;
    ...
    $page_content = $ROOT_DIR . "app/group_admin/views/post.view.php";
    ...
    try {
        ...
        if ($post_content) {
            ...
            foreach ($image_tags as $image_tag) {
                ...
                $real_content_photo_path = str_replace($HOST_NAME, "../../../", $content_photo_path);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["ACTIVATE_CODE"]) {
    ...
    if ($post_id != "") {
        ...
        $post_status = 1;
        $status = 1;
        $property = array("id" => $post_id, "post_status" => $post_status, "status" => $status);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["SEND_TO_TELEGRAM_CODE"]) {
    ...
    try {
        ...
        if ($category) {
            if (isset($category["telegram_token"]) && $category["telegram_token"] != "") {
                $telegram_token = $category["telegram_token"];
            }
            if (isset($category["telegram_id"]) && $category["telegram_id"] != "") {
...
//*************************************************************************************
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["check"]) && $_POST["check"] == $_SESSION["GET_CATEGORY_LIST_CODE"]) {

    page_access_check(array(1, 2), $HOST_NAME);
    csrf_validation_ajax($_POST["_csrf"]);
    $subjectId = $categoryId = 0;
    ...
    if ($categoryId != "") {
        ...
        $txt_category .= '<option value="0" >انتخاب کنید</option>';
        foreach ($rows as $result) {
            if ($categoryId != 0 && $categoryId == $result["id"])
                $txt_category .= '<option value="' . $result["id"] . '" selected >' . $result["name"] . '</option>';
            else
                $txt_category .= '<option value="' . $result["id"] . '" >' . $result["name"] . '</option>';
        }
    }
...
if (
    ...
    $html = view_to_string("_posts.php", "app/group_admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
