...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $resource_id=0;
    if (isset($url_resource_id)) {
        if (is_numeric($url_resource_id)) {
            $resource_id = $url_resource_id;
        }
    }
    $selected_resource = $database->resources()
        ->select("*")
        ->where("id = ?", $resource_id)
        ->fetch();

    subject_access_check_ajax($selected_resource["subject_id"],  array(1, 2), $HOST_NAME);
    
   $HOST_NAME = (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $SUBFOLDER_NAME;
    $ROOT_DIR = $_SERVER['DOCUMENT_ROOT'] . $SUBFOLDER_NAME;
    //and then call a template:
    $page_title = "همگروه - بانک اطلاعات عمومی";
    $page_content = $ROOT_DIR . "app/group_admin/views/resource_attachments.view.php";
...
if (
    ...
    if (isset($_POST["resource_id"])); {
        if (is_numeric($_POST["resource_id"]) && $_POST["resource_id"] != 0) {
    ...
    $html = view_to_string("_resource_attachments.php", "app/group_admin/views/partial/", $rows, $pagination, $SUBFOLDER_NAME, $HOST_NAME);
    echo $html;
    exit;
}
