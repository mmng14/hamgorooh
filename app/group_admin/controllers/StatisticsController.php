...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
     ...
     if ($_GET) {
          ...
          $subject_info = $database->subject()
               ->select("*")
               ->where("status = ?", 1)
               ->where("id = ?",  $url_subject_id)
               ->fetch();


          $statictic_subject_id = $subject_info["id"];
          $statictic_subject_name = $subject_info["name"];
          ...
          //$statistics_subject_id =  $url_subject_id;
          $page_content = $ROOT_DIR . "app/group_admin/views/statistics.view.php";
...
//*****************************   Get Statistics Data **********************************
//*************************************************************************************
if ($_SERVER['REQUEST_METHOD'] === 'POST' &&   isset($_POST["check"]) && $_POST["check"] == "group_admin_statistics_op_code") {
     ...
     $subjectCategories = $database->category()
          ->select("*")
          ->where("subject_id = ?", $subject_id);

     $categoriesWeeklyVisit =  _group_by($visits, "category_id");
     $categoriesWeeklyVisitData = array();
     foreach ($categoriesWeeklyVisit as $key => $value) {
          $keyName = getCategoryName($subjectCategories, $key);
          array_push($categoriesWeeklyVisitData, [$keyName, count($value)]);
     }


     $categoriesMonthlyVisitStr = "";
     foreach ($month_visits as $monthly_visit) {
          if ($monthly_visit["yearmonth"] == $year_month_int) {
               $categoriesMonthlyVisitStr = $monthly_visit["categories"];;
          }
     }
     $categoriesMonthlyVisitArr = explode(",", $categoriesMonthlyVisitStr);
     $categoriesMonthlyVisitData = array();
     if (count($categoriesMonthlyVisitArr) > 0) {
          foreach ($categoriesMonthlyVisitArr as $categoriesMonthlyItem) {
               $categoryMonthlyVisitArr = explode(":", $categoriesMonthlyItem);
               if (count($categoryMonthlyVisitArr) > 1) {
                    $keyId =  $categoryMonthlyVisitArr[0]; // category id                    
                    $keyName = getCategoryName($subjectCategories, $keyId);
                    $keyValue = $categoryMonthlyVisitArr[1];
                    array_push($categoriesMonthlyVisitData, [$keyName, (int)$keyValue]);
               }
          }
     }


     $lineChartCategores = ['SATTT', 'SUN', 'MON', 'TUS', 'WED', 'THR', 'FRI'];
     $lineChartData =  [800, 700, 500, 100, 50, 20, 10];
     ...
     echo json_encode(
          array(
               ...
               "categoriesWeeklyVisitData" => $categoriesWeeklyVisitData,
               "categoriesMonthlyVisitStr"=>$categoriesMonthlyVisitStr,
               "categoriesMonthlyVisitData" => $categoriesMonthlyVisitData,
               "visits" => $visits,
...
function getCategoryName($subjectCategories, $catId)
{
     $categoryName = "";
     if ($catId == 0) {
          $categoryName = "منابع";
     ...
     foreach ($subjectCategories as $subjectcategory) {
          if ($subjectcategory["id"] == $catId) {
               $categoryName = $subjectcategory["name"];
               return $categoryName;
          }
     }
     return $categoryName;
