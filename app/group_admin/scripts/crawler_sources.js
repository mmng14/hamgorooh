$(document).ready(function () {
  ...
  function getSubCategories() {
    ...
    if (checkInputs == true) {
      ...
      $(".loading-div").show();
      $.ajax({
        url: HOST_NAME + "group_admin/crawler-sources/",
        type: "POST",
        data: {
          s_id: $("#selected_subject_id").val(),
          c: cat_id,
          sc: 0,
          check: check,
          _csrf: $("#_csrf").val(),
  ...
  registerFormValidation = $("#formCrawlerSources").validate({
    rules: {
      category_type: {
        required: true,
        noHtmlTag: true,
...
window.onload = function () {
  ...
  $("#results").load(HOST_NAME + "group_admin/crawler-sources/", {
    cmd:  $("#listing_code").val(),//LISTING_CODE,
    _csrf: $("#_csrf").val(),
    sf: $("#selected_subject_id").val()
  ...
  $("#results").on("click", ".pagination a", function (e) {
    ...
    $("#results").load(
      HOST_NAME + "group_admin/crawler-sources/",
      {
        page: page,
        order: order,
        perpage: perpage,
...
function setData(id) {
  ...
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/crawler-sources/",
    data: "obj=" + id +"&s_id=" + subject_id + "&check=" + EDIT_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
...
function activate(id) {
  ...
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/crawler-sources/",
    data: "obj=" + id +"&s_id=" + subject_id + "&check=" + ACTIVATE_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
      if (result.state == "1") {
...
function deactivate(id) {
  ...
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/crawler-sources/",
    data: "obj=" + id +"&s_id=" + subject_id + "&check=" + DEACTIVATE_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
      if (result.state == "1") {
