$(document).ready(function () {
  $("#menu_post_attachments").addClass("active");
  ...
  registerFormValidation = $("#formPostAttachments").validate({
    rules: {
      title: {
        required: true,
        noHtmlTag: true,
...
window.onload = function () {
  ...
  $("#results").load(HOST_NAME + "group_admin/post-attachments/", {
    cmd:  $("#listing_code").val(),//LISTING_CODE,
    post_id : $("#selected_post_id").val(),
    subject_id : $("#post_subject_id").val(),
  ...
  $("#results").on("click", ".pagination a", function (e) {
    ...
    $("#results").load(
      HOST_NAME + "group_admin/post-attachments/",
      {
        page: page,
        order: order,
        perpage: perpage,
  ...
  if (checkInputs == true) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: HOST_NAME + "group_admin/post-attachments/",
          type: "POST",
          data: {
            obj: id,
            subject_id : $("#post_subject_id").val(),
...
function setData(id) {
  ...
  var _csrf = $("#_csrf").val();
  var subject_id = $("#post_subject_id").val();
  var category_id = $("#post_category_id").val();

  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/post-attachments/",
    data: "obj=" + id +"&subject_id=" + subject_id +"&category_id=" + category_id + "&check=" + EDIT_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
...
function getParentListData(id) {
  let get_drop_down_list = $("#get_category_list_code").val();
  var _csrf = $("#_csrf").val();
  var postId =  $("#selected_post_id").val();
  var subject_id = $("#post_subject_id").val();
  var category_id = $("#post_category_id").val();
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/post-attachments/",
    data: "id=" + id + "&postId=" + postId + "&subject_id=" + subject_id +"&category_id=" + category_id + "&check=" + get_drop_down_list + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
...
function activate(id) {
  ...
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/post-attachments/",
    data: "obj=" + id + "&subject_id=" + subject_id +"&category_id=" + category_id + "&check=" + ACTIVATE_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
      if (result.state == "1") {
...
function deactivate(id) {
  ...
  $.ajax({
    type: "POST",
    url: HOST_NAME + "group_admin/post-attachments/",
    data: "obj=" + id+ "&subject_id=" + subject_id +"&category_id=" + category_id  + "&check=" + DEACTIVATE_CODE + "&_csrf=" + _csrf,
    dataType: "json",
    cache: false,
    success: function (result) {
      if (result.state == "1") {
...
function sendingData() {
  ...
  if (validate) {
    ...
    data["post_id"] = $("#selected_post_id").val();
    data["subject_id"] = $("#post_subject_id").val();
