                    ...
                    <div class="col col-xl-4 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="ui-block">
                            ...
                            <div class="birthday-item inline-items" id="post_item_<?= $item["subject_id"]  ?>_<?= $item["id"]  ?>">
                                <?php
                                $link_post_id =   $item['id'];
                                $link_post_id_length =  strlen($item['id']);
                                ...
                                <div class="author-thumb">
                                    <img src="<?php echo $HOST_NAME . $item["thumb_address"]  ?>" alt="<?php echo $item["title"]  ?>" width="40">
                                </div>
                                <div class="birthday-author-name">
                                    <a href="<?= $link_post_url ?>" class="h6 author-name"><?php echo $item["title"]  ?> </a>
                                    <div class="birthday-date"> <?php echo $item["brief_description"]  ?> </div>
                                </div>

                                <div style="float: left;margin-top: 0px;">

                                    <!-- <span class="notification-icon"> -->
                                    <button onclick="acceptPost(<?= $item['subject_id']  ?>,<?= $item['id']  ?>)" data-subject-post-id="<?= $item['subject_id']  ?>_<?= $item['id']  ?>" title="تایید پست" class="accept-request">
                                    ...
                                    <button onclick="rejectPost(<?= $item['subject_id']  ?>,<?= $item['id']  ?>)" data-subject-post-id="<?= $item['subject_id']  ?>_<?= $item['id']  ?>" title="رد پست" class="accept-request request-del">
...
<script src="<?php echo $HOST_NAME; ?>app/group_admin/scripts/user_posts.js"></script>