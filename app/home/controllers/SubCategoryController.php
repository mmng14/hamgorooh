...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($subcategory_page)) {
        $page = (int) substr($subcategory_page, 1, strlen($subcategory_page) - 1);
        if($page < 1){
            $page = 1; 
        }
    } else {
    ...
    if (isset($subcategory_code)) {

        $subject_id_length = (int) substr($subcategory_code, 0, 1);
        $subject_id = (int) substr($subcategory_code, 1, $subject_id_length);
        $category_id_length = (int) substr($subcategory_code, 1 + $subject_id_length, 1);
        $category_id = (int) substr($subcategory_code, 1 + $subject_id_length + 1, $category_id_length);
        $subcategory_length = (int) substr($subcategory_code, 1 + $subject_id_length + 1 + $category_id_length, 1);
        $subcategory_id = substr($subcategory_code, 1 + $subject_id_length + 1 + $category_id_length + 1, $subcategory_length);
        ...
        $subcategory_url = $subcategory["url_name"];
        ...
        $base_url = $HOST_NAME . "subcategory/" . $subcategory_code . "/{$subcategory_url}/" . "p";
    ...
    $page_content = $ROOT_DIR . "app/home/views/subcategory.view.php";
