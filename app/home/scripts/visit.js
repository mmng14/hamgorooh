    ...
    if (start_focus_time != undefined) {
        ...
        if ( state == 1) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                data: "afid=" + afid + "&ftime=" + total_focus_time + "&sid=" + sid + "&cid=" + cid + "&scid=" + scid + "&pid=" + pid + "&puid=" + puid,
                url: "http://localhost:8080/hamgorooh.com/resources/ajax/ajax_visit.php",
                cache: false,
                success: function (result) {
                    // $('#afid').val(result.newafid);
                }
            });
        }
