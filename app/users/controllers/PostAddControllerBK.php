...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if (is_numeric($request_category_id)) {
            ...
            if (isset($url_post_id)) {
                ...
                $post = $database_subject->post()
                    ->select("id,subject_id,category_id,sub_category_id,user_id,post_name,title,keywords,brief_description,user_full_name,reg_date,photo_address,thumb_address,source_name,source_link")
                    ->where("id = ?", $post_id)
    ...
    $page_content = $ROOT_DIR . "app/users/views/post_add.view.php";
...
//        array(
//            "check" => $_POST["check"],
//            "session" => $_SESSION["POST_IMAGE_UPLOAD_CODE"],
//        )
//    );
//    exit;
//}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["INSERT_CODE"]) {
    ...
    if ($post_id == null || $post_id == '') {
        ...
        ///{$page_number
        $redirect =  "{$HOST_NAME}users/posts/{$this_category_id}";
        echo json_encode(
            array(
                "status" => '1',
                "message" => $msg_ok,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check']) && $_POST['check'] == $_SESSION["UPDATE_CODE"]) {
    ...
    //post access check 
    if($post["status"]==1 || $post["user_id"] != $_SESSION["user_id"]){
        echo json_encode(
            array(

                "result" => "0",
                "redirect" => "",
                "message" => "امکان بروزرسانی این پست وجود ندارد",
                "status" => "0",
            )
        );
