...
function setUploadFolder() {
  ...
  var check = $('#set_upload_folder_code').val();
  $.ajax({
    type: "POST",
    url: HOST_NAME + "users/post_add/",
    data:
      "s_id=" +
      subject_id +
      "&upload_folder=" +
...
function uploadImageToServer() {
  ...
  var uploadFolder = $("#_upload_folder").val();
  var check = $('#post_image_upload_code').val();
...
function selectFromPhotoList(id) {
  var savedImageAddress = $(".choose-photo-item #img_" + id).attr("src");
  if (savedImageAddress != undefined && savedImageAddress != "") {
    CKEDITOR.instances["description"].insertHtml(
      '<img src="' + savedImageAddress + '">'
    );
