...
Connect();
$rows=select('crawler_source','*',"status='2' AND subject_id=4","limit 1");
...
if(isset($rows) && $rows !="" && count($rows) >= 1) {
    foreach ($rows as $row) {
        ...
        echo "<hr><h1>Want to Delete Duplicates  </h1>";
        if($res)
        {
            $msg = update("crawler_source", "status=-1", "id={$row['id']}");
            //Delete duplate items
              
            $sql_query = "DELETE FROM crawler_items AS a INNER JOIN ( SELECT item_link, MIN( id ) AS min_id FROM crawler_items GROUP BY item_link HAVING COUNT( * ) > 1 ) AS b ON b.item_link = a.item_link AND b.min_id <> a.id";
            $del_dupicates = delete_by_sql("{$sql_query}","142GJgkjkloiepqepopqwe[w");
            if($del_dupicates){ echo "<hr><h1>Duplicates Deleted </h1>"; };
        }
    }
}
