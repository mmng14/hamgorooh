...
$rows=select('crawler_source','*',"status='1' AND subject_id={$SUBJECT_INDEX}","limit 1");
...
{
    //set crawler_source of links to 0
    $msg = update("crawler_source", "status=1", "subject_id={$SUBJECT_INDEX}  AND status=0");
}
...
function fill_crawler_items($sourceId,$category_type,$subject_id,$category_id,$sub_category_id,$crawler_get_items,$crawler,$source_name,$source_link)
{
    ...
    if($sourceId != '0')
    {
        ...
        try{
            ...
            {
                echo '<hr/>' .  $_SESSION["dblayer_query"].'<hr/>';
